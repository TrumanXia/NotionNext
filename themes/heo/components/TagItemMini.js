...
const TagItemMini = ({ tag, selected = false }) => {
  return (
    <Link
      ...
      <div className='font-bold flex items-center  px-2 py-1 rounded-2xl'><HashTag className='text-gray-500 stroke-2 mr-0.5 w-3 h-3'/> {tag.name + (tag.count ? `(${tag.count})` : '')} </div>
    </Link>
  )
}
